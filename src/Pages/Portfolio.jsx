...
import { Gamepad2, Award, Boxes, Tangent } from "lucide-react";
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const toolData = await Promise.all(
        toolSnapshot.docs.map(async (doc) => {
          return {
            id: doc.id,
            ...doc.data()
          };
      ...
      // const toolData = toolSnapshot.docs.map((doc) => ({
      //   id: doc.id,
      //   ...doc.data(),
      //   TechStack: doc.data().TechStack || [],
      //   Contents: "Hello World Initial Conetnt"
      // }));
          ...
          {/* <TabPanel value={value} index={1} dir={theme.direction}>
            <div className="container mx-auto flex justify-center items-center overflow-hidden">
              <div className="grid grid-cols-1 md:grid-cols-3 md:gap-5 gap-4">
                {displayedTools.map((tools, index) => (
                  <div
                    key={index}
                    data-aos={
                      index % 3 === 0
                  ...
                  >
                    <ToolsDetails ImgSertif={tools.Img} />
                  </div>
                ))}
              </div>
            </div>
            {toolsets.length > initialItems && (
              <div className="mt-6 w-full flex justify-start">
                <ToggleButton
                  onClick={() => toggleShowMore("tools")}
                  isShowingMore={showAllTools}
